...
export class CardInfo extends ChildComponent {
	...
	fillElements() {
		...
		$ML(this.element).find('#card-expire-date').text(this.card.expireDate)

		const cardCvcElement = $ML(this.element).find('#card-cvc')
	...
	render() {
		if (this.store.state.user) {
			$ML(this.element).html(new Loader().render().outerHTML)
